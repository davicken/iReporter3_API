...
class UsersController:
    ...
    #                 "Users Endpoints",
    #                 "#1 : GET /api/v1/users",
    #                 "#2 : GET /api/v1/users/<red_flag_id>",
    #                 "#3 : POST /api/v1/users",
    #                 "#4 : PATCH /api/v1/users/<red_flag_id>/location",
    #                 "#5 : PATCH /api/v1/users/<red_flag_id>/comment",
    #                 "#6 : DELETE /api/v1/users/<red_flag_id>"
    #                 ]
    #         })
    ...
    def make_user_admin(self, user_id):
        # up grade/promote user role to admin
    ...
    def login(self):
        ...
        for user in my_users.users_list:
            ...
            if not user.__dict__["userName"] == username:
                return jsonify({
                    "status": 400,
                    "error": "The Username you provided is doesn\'t exist, try again with a correct one!"
                }), 400
            if check_password_hash(user.__dict__["password"], password_input):
                # generate the token
                token = jwt.encode({'user_id': user.__dict__["userId"], 'exp':datetime.datetime.utcnow() + datetime.timedelta(minutes=30)}, 
